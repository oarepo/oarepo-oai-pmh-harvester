...
from oarepo_oaipmh_harvester.oai_batch import config
...
class Oai_batchExt:
    ...
    def service_records(self):
        return config.OAI_BATCH_RECORD_SERVICE_CLASS(
            config=config.OAI_BATCH_RECORD_SERVICE_CONFIG(),
        )

    @cached_property
    def resource_records(self):
        return config.OAI_BATCH_RECORD_RESOURCE_CLASS(
            service=self.service_records,
            config=config.OAI_BATCH_RECORD_RESOURCE_CONFIG(),
        )
